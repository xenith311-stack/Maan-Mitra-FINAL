...
export function AICompanion({ navigateTo, userData }: AICompanionProps = {}) {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          ...
          if (userLanguagePref === 'hindi') {
            languageCode = 'hi-IN';
            alternativeLanguageCodes = ['en-IN']; // Hindi primary, English secondary
          } else if (userLanguagePref === 'english') {
            languageCode = 'en-IN';
            alternativeLanguageCodes = ['hi-IN']; // English primary, Hindi secondary
          } else { // 'mixed' preference
            languageCode = 'en-IN'; // Default to English primary for mixed
            alternativeLanguageCodes = ['hi-IN']; // Hindi secondary
            // Or prioritize Hindi if needed:
            // languageCode = 'hi-IN';
  ...
  return (
    ...
    <div className="flex h-screen relative bg-gradient-to-br from-background via-secondary/30 to-primary/10">
      {/* Sidebar */}
      <div className={`absolute lg:relative top-0 left-0 h-full w-64 bg-background/95 backdrop-blur-sm border-r border-border shadow-xl z-20 transition-transform duration-300 ease-in-out ${showConversationList ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
      ...
      <div className="relative flex-1 flex flex-col h-screen"> {/* Ensure chat area takes full height */}
        ...
        <div className="relative flex-1 p-6 sm:p-8 lg:p-8 flex flex-col h-full max-h-screen">
          <div className="max-w-4xl mx-auto w-full flex flex-col h-full">
            {/* Header (adjusted for toggle button) */}
            <div className="flex items-center mb-8 pl-12 lg:pl-0"> {/* Add padding left for toggle */}
