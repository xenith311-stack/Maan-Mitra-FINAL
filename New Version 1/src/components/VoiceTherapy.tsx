...
export default function VoiceTherapy() {
  const { userProfile } = useAuth(); // Get user profile for language preferences
  ...
  // Initialize transcribeAudio function
  ...
  // Handle voice input for chat mode
  const handleChatVoiceInput = async (transcript: string) => {
  ...
  const startListening = async () => {
    ...
    // *** Determine Language Codes from User Preference ***
    const userLanguagePref = userProfile?.preferences?.language || 'mixed'; // Default to mixed
    let languageCode = 'en-IN'; // Default primary
    let alternativeLanguageCodes: string[] = ['hi-IN']; // Default secondary

    // Language mapping for currently supported preferences
    // Note: UserProfile currently supports 'hindi' | 'english' | 'mixed'
    // Future: Expand UserProfile interface to support more Indian languages
    if (userLanguagePref === 'hindi') {
      languageCode = 'hi-IN';
      alternativeLanguageCodes = ['en-IN'];
    } else if (userLanguagePref === 'english') {
      languageCode = 'en-IN';
      alternativeLanguageCodes = ['hi-IN'];
    } else { // Default for 'mixed' or unmapped codes
      languageCode = 'en-IN'; // Prioritize English for mixed
      alternativeLanguageCodes = ['hi-IN'];
    }

    console.log(`STT Language Setup - User Pref: ${userLanguagePref}, Primary: ${languageCode}, Alternatives: [${alternativeLanguageCodes.join(', ')}]`);
    ...
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          ...
          console.log(`Calling transcribeAudio (Pref: ${userLanguagePref}, Primary: ${languageCode}, Alts: [${alternativeLanguageCodes.join(', ')}])...`);
          ...
          if (transcript) {
            if (isInChatMode) {
              // Handle chat mode - process as chat message
              await handleChatVoiceInput(transcript);
            } else {
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (isListening) {
                        ...
                        // Use the unified STT approach for chat mode
                        startListening();
                      }
                    }}
                    disabled={isPlaying}
