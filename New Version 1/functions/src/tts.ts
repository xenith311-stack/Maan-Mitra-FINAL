...
export const synthesizeSpeech = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
    ...
    if (voiceName.includes('-Wavenet-A') || voiceName.includes('-Wavenet-C') || voiceName.includes('-Standard-A') || voiceName.includes('-Standard-C')) {
        ssmlGender = protos.google.cloud.texttospeech.v1.SsmlVoiceGender.FEMALE;
    } else if (voiceName.includes('-Wavenet-B') || voiceName.includes('-Wavenet-D') || voiceName.includes('-Standard-B') || voiceName.includes('-Standard-D')) {
