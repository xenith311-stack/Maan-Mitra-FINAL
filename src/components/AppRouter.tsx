  ...
  return (
    <>
      ...
      <div className="flex h-screen relative">
        ...
        <div className={`fixed top-0 left-0 h-full z-50 transform transition-transform duration-300 ease-in-out md:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}>
          <NavigationSidebar
            currentUser={currentUser}
            onLogout={handleLogout}
        ...
        <div className="flex-1 w-full overflow-auto">
          {/* Mobile header */}
          <div className="md:hidden bg-white border-b border-green-100 p-4 flex items-center justify-between sticky top-0 z-30">
            <button
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="font-bold text-lg text-green-600">MannMitra</h1>
            </div>
...
const NavigationSidebar: React.FC<{
...
}> = ({ currentUser, onLogout, systemStatus, sidebarOpen, setSidebarOpen }) => {
  const navigate = useNavigate();
  ...
  return (
    <div className="w-64 h-screen flex flex-col bg-white shadow-lg border-r border-gray-200" style={{
      ...
      {/* Header */}
      <div className="p-4 md:p-6 relative" style={{ borderBottom: '1px solid rgba(34, 197, 94, 0.15)' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-green-400 via-green-500 to-green-600 rounded-xl flex items-center justify-center shadow-lg relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent"></div>
              <span className="text-white text-lg md:text-xl relative z-10">❤️</span>
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-300 rounded-full animate-pulse"></div>
            </div>
            <div>
              <h1 className="font-bold text-lg md:text-xl bg-gradient-to-r from-green-600 to-green-700 bg-clip-text text-transparent">MannMitra</h1>
              <p className="text-xs md:text-sm font-medium" style={{ color: '#059669' }}>AI Mental Health Companion</p>
            </div>
          </div>
          {/* Mobile close button */}
          <button
        ...
        <div className="absolute bottom-0 left-4 right-4 md:left-6 md:right-6 h-px bg-gradient-to-r from-transparent via-green-200 to-transparent"></div>
      </div>

      {/* User Profile */}
      {currentUser && (
        <div className="p-3 md:p-4 relative" style={{ borderBottom: '1px solid rgba(34, 197, 94, 0.15)' }}>
          <div className="flex items-center gap-2 md:gap-3 p-2 md:p-3 rounded-xl bg-gradient-to-r from-green-50/80 to-emerald-50/60 border border-green-100/50">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-green-100 to-green-200 rounded-full flex items-center justify-center shadow-sm border border-green-200/50">
              <span className="text-green-600 text-sm md:text-lg">👤</span>
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-xs md:text-sm truncate" style={{ color: '#065f46' }}>{currentUser.displayName}</p>
              <p className="text-xs font-medium truncate" style={{ color: '#059669' }}>{currentUser.email}</p>
            </div>
      ...
      {/* System Status */}
      <div className="p-3 md:p-4 relative" style={{ borderBottom: '1px solid rgba(34, 197, 94, 0.15)' }}>
        <h3 className="text-xs font-bold mb-2 md:mb-3 uppercase tracking-wider" style={{ color: '#059669' }}>System Status</h3>
        <div className="space-y-2 md:space-y-3">
          <div className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-green-50/50 to-emerald-50/30">
            ...
            <span className={`text-xs px-1.5 md:px-2 py-1 rounded-full font-semibold ${systemStatus.aiOrchestrator ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200'}`}>
          ...
          <div className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-green-50/50 to-emerald-50/30">
            ...
            <span className={`text-xs px-1.5 md:px-2 py-1 rounded-full font-semibold ${systemStatus.voiceAnalysis ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-gray-100 text-gray-800 border border-gray-200'}`}>
          ...
          <div className="flex items-center justify-between p-2 rounded-lg bg-gradient-to-r from-green-50/50 to-emerald-50/30">
            ...
            <span className={`text-xs px-1.5 md:px-2 py-1 rounded-full font-semibold ${systemStatus.emotionDetection ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-gray-100 text-gray-800 border border-gray-200'}`}>
      ...
      {/* Navigation Menu */}
      <nav className="flex-1 bg-white p-3 md:p-4 overflow-y-auto">
        <div className="space-y-1 md:space-y-2">
          {menuItems.map((item) => (
            <button
              key={item.route}
              className={`w-full text-left px-3 md:px-4 py-2.5 md:py-3 rounded-xl text-sm transition-all duration-200 cursor-pointer relative overflow-hidden group ${location.pathname === item.route
            ...
            >
              <span className="flex items-center gap-2 md:gap-3 relative z-10">
                <span className="text-lg md:text-xl transition-transform duration-200 group-hover:scale-110 flex-shrink-0">{getIcon(item.icon)}</span>
                <span className="font-medium truncate" style={{ color: location.pathname === item.route ? '#15803d' : '#374151' }}>{item.label}</span>
                {location.pathname === item.route && (
                  <div className="absolute right-0 w-1 h-4 md:h-6 bg-green-500 rounded-full"></div>
                )}
              </span>
      ...
      {/* Footer */}
      <div className="p-3 md:p-4" style={{ borderTop: '1px solid rgba(34, 197, 94, 0.15)' }}>
        <button
          className="w-full px-3 md:px-4 py-2.5 md:py-3 rounded-xl text-sm font-medium transition-all duration-200 hover:bg-red-50 hover:border-red-200 hover:text-red-600 hover:shadow-md group relative overflow-hidden"
          style={{
        ...
        <div className="bg-gradient-to-br from-white/95 to-green-50/80 rounded-2xl shadow-xl p-8 border border-green-100/50 backdrop-blur-sm relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-6">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const updatedUser = {
                      ...
                      preferences: {
                        ...user.preferences,
                        language: e.target.value as 'hindi' | 'english' | 'mixed' | 'bengali' | 'marathi' | 'telugu' | 'tamil' | 'gujarati' | 'kannada' | 'malayalam' | 'urdu' | 'punjabi' | 'odia' | 'assamese'
                      }
                    };
              ...
              <div className="p-6 border rounded-2xl bg-gradient-to-br from-white to-green-50/30 shadow-lg border-green-100/60">
                <label className="block text-sm font-bold mb-3 text-green-700 uppercase tracking-wide">App Theme</label>
                <p className="text-sm text-gray-600 mb-4">Choose your preferred visual theme for the application</p>
                <ThemeSelector showTitle={false} compact={false} className="mt-4" />
              </div>
